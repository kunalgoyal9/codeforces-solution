...
int main(void) {
	...
	*/
	...
	fors(i,0,str.length()){
		if(str[i]!='a' and str[i]!='b'){
			...
			newstr += str[i];
				...
				if(newstr.length()!=0){
					if(newstr[newstr.length()-1]=='b') continue;
					else{
						newstr += 'b';
					}
				}
	...
	if(newstr.length()!=0 and newstr[newstr.length()-1]=='b'){
		fors(i,0,newstr.length()-1){
			fstr += newstr[i];
		}
	}
	else{
	...
	lli ans = arr[0];
	fors(i,1,arr.size()){
		ans = (ans + ((lli)arr[i]*(lli)(ans+1))%mod1)%mod1;
	}
