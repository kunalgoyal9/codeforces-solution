...
int main(void) {
	...
	*/
	...
	map<pair<int,int>,int> mchk, midx;
	...
	fors(i,0,n){
		...
		int fa=b[0].first+c[i].first,fb=b[0].second+c[i].second;
		//trace2(fa,fb);
		mpp[i] = 1;
		int chk = true;
		
		fors(j,1,n){
			int cx = fa - b[j].first, cy = fb - b[j].second;
			////trace2(cx,cy);
			//trace1(int(mchk[{cx,cy}]));
			if((int(mchk[{cx,cy}]) == 1) and (mpp[midx[{cx,cy}]]==0)){
