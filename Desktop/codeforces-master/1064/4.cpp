...
int main(void) {
	...
	*/
	...
	while(!q.empty()){
		int tr,tc, cx, cy;
		tr = q.front().first.first;
		tc = q.front().first.second;
		cx = q.front().second.first;
		cy = q.front().second.second;
		cerr<<tr<<' '<<tc<<' '<<cx<<' '<<cy<<endl;
		q.pop();
		
		if(cx<=x and cy<=y and arr[tr][tc]=='.'){
			//cerr<<tr<<' '<<tc<<' '<<cnt<<endl;
			//cerr<<cx<<' '<<cy<<endl;
			cnt++;
		}
		fors(i,0,4){
			int ntr = tr+dx[i];
			int ntc = tc+dy[i];
			if(ntr<n and ntr>=0 and ntc<m and ntr>=0 and vis[ntr][ntc] == 0){
				vis[ntr][ntc] = 1;
				cerr<<ntr<<' '<<ntc<<endl;
				cerr<<vis[ntr][ntc]<<endl;
				if(arr[ntr][ntc]=='*') continue;
				if(dy[i]==1){
					if(cy+1 <= y){
						q.push({{ntr,ntc},{cx,cy+1}});
					}
				}
				else if(dy[i]==-1){
					if(cx+1 <= x){
						q.push({{ntr,ntc},{cx+1,cy}});
					}
				}
				else{
					q.push({{ntr,ntc},{cx,cy}});
				}
			}
		}
	}
