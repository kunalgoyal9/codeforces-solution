...
//int dx[]={1,-1,0,0}; int dy[]={0,0,1,-1};
 ...
 lli common(int lb, int rb, int la, int ra){
	 if(la>rb or lb>ra){
		 return 0;
	 }
	 else{
		 return abs(min(la,lb)-min(ra,rb));
	 }
 }
...
int main(void) {
	...
	*/
	...
	while(coutt--){
		...
		lli k_1 = floor((lb-la)/ta);
		lli nla = la + k_1*ta;
		lli nra = ra + k_1*ta;
		
		//trace2(lb,rb);
		//trace2(nla,nra);
		 maxm = max(common(lb,rb,nla,nra),maxm);
		//trace1(maxm);
		
		lli k1 = floor((lb-la)/ta)+1;
		 nla = la + k1*ta;
		 nra = ra + k1*ta;
