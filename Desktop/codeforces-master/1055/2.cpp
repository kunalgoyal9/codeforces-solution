...
int main(void) {
	...
	*/
	...
	fors(i,0,n-1){
		if(barr[i]==1 and barr[i+1]==0) cnt++;
	}
	if(barr[n-1]==1) cnt++;
	...
	fors(i,0,m){
		...
		if(t==0){
			...
			if(p==0){
				...
				newp = arr[p]+d;
				bool chk = false;
				if(newp>l and arr[p]<=l){
					chk = true;
					barr[p] = 1;
				}
				arr[p] = newp;
				if(barr[1]==0 and chk) cnt++;
				//trace2(p,cnt);
			}
			else if(p==n-1){
				lli newp;
				...
				arr[p] = newp;
				if(chk and (barr[p-1]==0 and barr[p+1]==0)){
					cnt++;
				}
				else if(chk and (barr[p-1]==1 and barr[p+1]==1)){
					cnt--;
				}
				//trace3(p,cnt,chk);
			}
