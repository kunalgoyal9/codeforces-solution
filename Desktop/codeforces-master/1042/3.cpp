...
int main(void) {
	...
	*/
	...
	if(pos.size()!=0 and neg.size()!=0 and zer.size()!=0){
		int ns = neg[0];
		fors(i,1,neg.size()){
			cout<<1<<' '<<neg[i]<<' '<<ns<<endl;
		}
		cout<<2<<' '<<ns<<endl;
		fors(i,0,zer.size()){
			pos.pb(zer[i]);
		...
		int ps = pos[0];
		fors(i,1,pos.size()){
			cout<<1<<' '<<pos[i]<<' '<<ps<<endl;
		}
	}
	else if(pos.size()!=0 and neg.size()!=0 and zer.size()==0){
		int ns = neg[0];
		fors(i,1,neg.size()){
			cout<<1<<' '<<neg[i]<<' '<<ns<<endl;
		}
