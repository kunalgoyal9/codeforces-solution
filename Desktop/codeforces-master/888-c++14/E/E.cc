...
int main(void) {
	...
	lli maxm = -INF;
	fors(i,v1.size()){
		lli t1 = lower_bound(all(v2),m-v1[i]-1)-v2.begin();
		if(*lower_bound(all(v2),m-v1[i]-1)!=m-v1[i]-1) --t1;
		maxm = max(maxm,(v2[t1]+v1[i])%m);
		lli t2 = lower_bound(all(v2),2*m-v1[i]-1)-v2.begin();
		if(*lower_bound(all(v2),2*m-v1[i]-1)!=2*m-v1[i]-1) --t2;
		//ps;plint(2*m-v1[i]-1);ps;
		maxm = max(maxm,(v2[t2]+v1[i])%m);
	}
	plint(maxm);
    return 0;
}
