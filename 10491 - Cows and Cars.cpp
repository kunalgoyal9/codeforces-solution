...
int main(void) {
    ...
    int n,m,k;
    while(int3(n,m,k)==3){
        double nd = (double)n, md = (double)m, kd=(double)k;
        double first = (md*(nd+md-1))/((nd+md)*(nd+md-kd-1));
        double second = md/(md+nd);
        double maxm = max(first,second);
        cout<<fixed<<setprecision(5)<<maxm<<endl;
    }
